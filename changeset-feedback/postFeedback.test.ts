...
- **@kozmoai/app-defaults**

See [CONTRIBUTING.md](https://github.com/kozmoai/glint/blob/master/CONTRIBUTING.md#creating-changesets) for more information about how to add changesets.
...
|:-------------|:-------------|:--------------:|:----------------|
| @kozmoai/app-defaults | packages/app-defaults | **none** | \`v1.0.9-next.1\` |
`;
...
const body = (feedback: string) =>
  feedback.trim() ? feedback + marker : undefined;
...
describe('changeset feedback', () => {
  ...
  it('should skip updating comment', async () => {
    ...
    expect(log).toHaveBeenCalledWith(
      'skipped update of identical comment in #1',
    );
  });
